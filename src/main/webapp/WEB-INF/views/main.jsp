	...
	<div class="listView clearfix">
	    ...
	    <div class="main_rankList">
	        ...
	        <div class="rating" id="rating_${status.index}_rank">
	            <div class="stars" id="stars_${status.index}" style="flex-direction: row-reverse;">
			...
			<p class="star-score" style="margin-bottom: 0;">${rank.stars}</p>   
...
<div class="area_div" >
    ...
    <div class="areaListView clearfix">
        ...
        <div class="main_areaList">
            ...
            <p class="star-score">${aa.stars}</p>

            <script>
                // 별점 셋팅 스크립트
                (function() {
                    let astars = Math.round(parseFloat('${aa.stars}')); // 평균 별점을 반올림
                    let radio = document.querySelector('input[name="rating_area_${status.index}"][value="' + astars + '"]');
                    if (radio) radio.checked = true; // 해당 별을 체크
                })();
            </script>
        </div>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const areaListView = document.querySelector('.areaListView');

    // 초기에는 지역 리스트 숨기기
    areaListView.style.display = 'none';

    areaButtons.forEach(btn => {
      btn.addEventListener('click', function () {
        const areaName = this.value;
        selectedAreaText.textContent = areaName + ' 추천 맛집';
        areaListView.style.display = 'flex'; // 버튼 클릭 시 리스트 보이기
      });
    });
  });
</script>
